...
export const Button: React.FC<PropsButton> = ({
    ...
    return (
        <div>
            {deleteButton ?
                (<button
                    className={`${fullWidth && 'w-full flex justify-center'} text-white bg-red-600 hover:bg-red-800 focus:ring-4 focus:outline-none focus:ring-red-300 font-medium rounded-lg text-sm w-full sm:w-auto py-3 px-5 text-center`}
                    onClick={onClick}
                >
                ...
                secondary ? (
                    <button className={`${fullWidth && 'w-full justify-center'}flex py-2 px-4 rounded border rounded-lg border-black align-center justify-center`}
                        onClick={onClick}>
